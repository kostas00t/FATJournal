...
int test_creat(char *str, int len)
{
	...
	// --------------
	printf("========================================\n");
	printf("Create filejournal at /tmp/filejournal\n");
	int filejournal = open("/tmp/filejournal", O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO );
	printf("int value of filejournal: %d\n", filejournal);
	printf("========================================\n");
	// --------------
...
static int test_chdir(char *str, int len, const char *path)
{
	...
	int file = lkl_sys_open("file1111111.txt", LKL_O_CREAT, 0);
	//lkl_system("ls");
	snprintf(str, len, "%ld", ret);
